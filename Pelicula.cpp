...
void Pelicula::Mostrar() const {
    std::cout << "Pelicula: " << GetNombre() << std::endl <<"Duracion: " << GetDuracion() << std::endl <<"Genero: " << 
        GetGenero() << std::endl <<"ID: " << GetId() << std::endl <<"Calificacion Promedio: " << CalcularPromedio() << std::endl;
}
// Sobrecarga de operadores para mayor eficiencia
std::ostream& operator<<(std::ostream& os, const Pelicula& pelicula) {
    os << "Pelicula: " << pelicula.GetNombre() << std::endl<< "Duracion: " << pelicula.GetDuracion() << std::endl<< "Genero: " 
        << pelicula.GetGenero() << std::endl<< "ID: " << pelicula.GetId() << std::endl<< "Calificacion Promedio: " 
        << pelicula.CalcularPromedio() << std::endl;
    return os;
}
